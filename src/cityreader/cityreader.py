...
def cityreader(cities=[]):
  ...
  # `cities` list
	with open('cities.csv', newline='') as csvfile:
		...
		for row in reader:
			# Returning error data if erroneously formatted
			try:
				city = City(row['city'], float(row['lat']), float(row['lng']))
				cities.append(city)
			except ValueError:
				city = City('Error', 'Error', 'Error')
	return cities
...
def cityreader_stretch(lat1, lon1, lat2, lon2, cities=[]):
	...
	# using min(l1, l2) < x < max(l1, l2) takes care of issues with order
	within = [city for city in cities if
 			  (min(lat1, lat2) < city.lat < max(lat1, lat2))
